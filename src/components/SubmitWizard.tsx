...
const MAX_RETRIES = 6; // 6 försök × 5 sekunder = 30 sekunder totalt
...
export function SubmitWizard({ mode = 'create', appId, initialData }: SubmitWizardProps) {
  ...
  // Polling effect - retry loading image every 5 seconds
  useEffect(() => {
                ...
                {form.watch('url') && form.watch('url').startsWith('https://') && (
                  <div className="space-y-3 p-4 rounded-lg bg-muted/50 border border-border">
                      ...
                      {previewLoading && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-muted z-10">
                          ...
                          <p className="text-xs text-muted-foreground mt-2">
                            Laddar... (försök {retryCount + 1} av {MAX_RETRIES})
                          </p>
                        </div>
                      )}
                      {previewError && !previewLoading && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-muted z-10 text-center p-4">
                          ...
                          <p className="text-xs text-muted-foreground mt-1">
                            Du kan ladda upp en egen bild i steg 3
                          </p>
                        </div>
                      )}
                      <img
                        key={retryCount}
                        src={`${getPreviewUrl(form.watch('url'))}&retry=${retryCount}`}
                        alt="Förhandsvisning"
                        ...
                        onError={() => {
                          // Don't set error during polling, let it retry
                    ...
                    <p className="text-xs text-muted-foreground">
                      {previewError 
                        ? 'Automatisk bild misslyckades. Ladda upp en egen bild i steg 3.'
                        : previewLoading
                          ? 'Genererar bild från URL...'
                          : 'Denna bild används som thumbnail. Du kan byta till egen bild i steg 3.'}
                    </p>
                  </div>
                )}
                    ...
                    <FormItem>
                      ...
                      <FormDescription>
                        {isAutoGeneratedImage(field.value) 
                          ? 'En förhandsvisning genererades automatiskt från appens URL'
